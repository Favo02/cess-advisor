...
<div class="w-[400px] mx-auto min-h-40 p-4">
  <div class="block bg-base-100 shadow-lg shadow-primary/20 rounded-lg overflow-hidden">
    ...
    <div class="p-4">
      <h2 class="mt-2 mb-2 font-bold text-primary"><a href="/toilets/{id}" class="link">{title || "Unnamed toilet"}</a>
        {#if reviews_count > 0}
    ...
    <div class="flex justify-center align-middle gap-4">
      {#if reviews_count > 0}
        <a href="/toilets/{id}" class="btn btn-primary btn-outline">View reviews</a>
      {/if}
      <a href="/toilets/{id}/review" class="btn btn-primary">Review</a>
    </div>
